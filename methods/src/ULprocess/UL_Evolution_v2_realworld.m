function [saved_result, net] = UL_Evolution_v2_realworld(Params, prob, generation_pop, strategy, funchn, termination_criterion, net, saved_result, gen_gap, varargin)
%-
...
for gen = 1 : (Params.N_gen_UL - 1) % initialization caused one generation used
    ...
    if update_net_decision(strategy.name, gen+1, net, gen_gap)
        ...
        %         if size(net.Trg_LHS, 1) > 4999 && strategy.name == "net2skipLLsearch" % if 5000 points are not accumulated, especially when D=2
        %             seed = varargin{1};
        %             accuracy_experiment(13, prob, strategy, net, seed, 0);
        %             saved_result = [];
        %             return
        %         end
...
%             hold on;
%             scatter(FL(:, 1), FL(:, 2), 80, 'filled', 'DisplayName', 'PSP-BLEMO');
%             scatter(fl(:, 1), fl(:, 2), 80, 'filled', 'DisplayName','cG-BLEMO');
%             ax = gca;
...
clf(f1);
scatter(pf(:, 1), pf(:, 2), 20, [0 0.4470 0.7410], 'filled', 'DisplayName','UL PF'); hold on;
scatter(nd(:, 1), nd(:, 2), 40, [0.8500 0.3250 0.0980],  'DisplayName', 'ND front');
%pop = pop .* -1;
scatter(pop(:, 1), pop(:, 2) , 20, [0.9290 0.6940 0.1250], 'filled' , 'DisplayName', 'Population');
t = strcat('Generation: ', num2str(gen)); %, ', ', num2str(saved_result{1}), ', ', num2str(saved_result{2}));
title(t, 'FontSize', 20);
...
ylabel('F2', 'FontSize', 20, 'Rotation',0);
box on;
pause(0.1);
